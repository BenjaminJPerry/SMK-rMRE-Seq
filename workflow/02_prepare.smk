...
def get_passing_samples(wildcards):
    ...
    qc_passed = qc_stats.loc[qc_stats["num_seqs"].astype(int) > 100000]
    passed = qc_passed['file'].str.split("/").str[-1].str.split(".").str[0].tolist()
    return expand("results/01_prepare/{samples}.bbduk.fastq.gz", samples = passed)


rule seqkit_prepared:
    input:
        bbdukReads = get_seqkitMaskingBBDukReads_passing_samples,
    output:
        'results/00_QC/seqkit.report.prepared.txt'
    benchmark:
        'benchmarks/seqkit_prepared.txt'
    conda:
        'seqkit'
    threads: 32
    resources:
